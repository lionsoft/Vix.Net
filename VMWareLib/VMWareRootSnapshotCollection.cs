    ...
    public class VMWareRootSnapshotCollection : VMWareSnapshotCollection
    {
        ...
        protected override List<VMWareSnapshot> Snapshots
        {
            get
            {
                if (_snapshots == null)
                {
                    var snapshots = new List<VMWareSnapshot>();
                    int nSnapshots;
        ...
        {
            ISnapshot snapshot;
            var rc = _vm.GetNamedSnapshot(name, out snapshot);
            switch (rc)
            {
                case Constants.VIX_E_SNAPSHOT_NOTFOUND:
                    break;
        ...
        public void CreateSnapshot(string name, string description, int flags, int timeoutInSeconds)
        {
            ...
            var job = new VMWareJob(_vm.CreateSnapshot(name, description, 0, null, callback), callback);
