...
namespace Vestris.VMWareLib
{
    ...
    /// </summary>
    public class VMWareSharedFolderCollection : ICollection<VMWareSharedFolder>
    {
        private readonly IVM _vm;
        private List<VMWareSharedFolder> _sharedFolders;
        ...
        public void Add(VMWareSharedFolder sharedFolder)
        {
            ...
            var job = new VMWareJob(_vm.AddSharedFolder(
        ...
        private List<VMWareSharedFolder> SharedFolders
        {
            get
            {
                ...
                {
                    var sharedFolders = new List<VMWareSharedFolder>();
                    var callback = new VMWareJobCallback();
                    var job = new VMWareJob(_vm.GetNumSharedFolders(callback), callback);
                    
                    var nSharedFolders = job.Wait<int>(
                    ...
                    {
                        var getSharedfolderCallback = new VMWareJobCallback();
                        var sharedFolderJob = new VMWareJob(
                        ...
                        var sharedFolderPropertyValues = sharedFolderJob.Wait<object[]>(
                            sharedFolderProperties, VMWareInterop.Timeouts.GetSharedFoldersTimeout);

                        var sharedFolder = new VMWareSharedFolder(
        ...
        {
            var callback = new VMWareJobCallback();
            var job = new VMWareJob(_vm.RemoveSharedFolder(item.ShareName, 0, callback), callback);
            job.Wait(VMWareInterop.Timeouts.AddRemoveSharedFolderTimeout);
