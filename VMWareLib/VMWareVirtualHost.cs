...
namespace Vestris.VMWareLib
{
    ...
    public class VMWareVirtualHost : VMWareVixHandle<IHost>, IDisposable
    {
        ...
        {
            var serviceProvider = (int)serviceProviderType;
            var callback = new VMWareJobCallback();
            var job = new VMWareJob(VMWareInterop.Instance.Connect(
                Constants.VIX_API_VERSION, serviceProvider, hostName, hostPort,
                username, password, 0, null, callback), callback);

            Handle = job.Wait<IHost>(Constants.VIX_PROPERTY_JOB_RESULT_HANDLE, timeout);
        ...
        public VMWareVirtualMachine Open(string fileName, int timeoutInSeconds)
        {
            ...
            var callback = new VMWareJobCallback();
            var job = new VMWareJob(Handle.OpenVM(fileName, callback), callback);
        ...
        public IEnumerable<VMWareVirtualMachine> RunningVirtualMachines
        {
            get
            {
                ...
                var callback = new VMWareJobCallback();
                var job = new VMWareJob(Handle.FindItems(
                ...
                {
                    yield return Open((string) runningVirtualMachine[0]);
                }
            }
        }
